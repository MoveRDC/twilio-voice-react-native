...
import { createNativeCallInviteInfo, createMockNativeCallInviteEvents, } from '../__mocks__/CallInvite';
...
let MockInvalidStateError;
let MockCall;
let MockOutgoingCallMessage;
jest.mock('../common');
jest.mock('../Call', () => ({
...
describe('CallInvite class', () => {
    describe('constructor', () => {
        it('uses the passed CallInvite info', () => {
            const callInvite = new CallInvite(createNativeCallInviteInfo(), CallInvite.State.Pending);
            expect({
        ...
        it.each(Object.values(CallInvite.State))('uses the passed state "%s"', (callInviteState) => {
            const callInvite = new CallInvite(createNativeCallInviteInfo(), callInviteState);
            // eslint-disable-next-line dot-notation
    ...
    describe('when a native call invite event occurs', () => {
        ...
        describe('when the call sid matches', () => {
            const eventTest = (nativeEvent, event) => {
                const callInvite = new CallInvite(createNativeCallInviteInfo(), CallInvite.State.Pending);
                const spy = jest.fn();
            ...
            describe(mockNativeCallInviteEvents.accepted.type, () => {
                it('emits a call', () => {
                    const { spy } = eventTest(mockNativeCallInviteEvents.accepted, CallInvite.Event.Accepted);
                ...
                it('sets the call invite state to accepted', () => {
                    const { callInvite } = eventTest(mockNativeCallInviteEvents.accepted, CallInvite.Event.Accepted);
            ...
            describe(mockNativeCallInviteEvents.cancelled.type, () => {
                it('re-emits the event', () => {
                    const { spy } = eventTest(mockNativeCallInviteEvents.cancelled, CallInvite.Event.Cancelled);
                    expect(spy.mock.calls).toEqual([[]]);
                });
                it('emits an error when present', () => {
                    const { spy } = eventTest(mockNativeCallInviteEvents.cancelledWithError, CallInvite.Event.Cancelled);
                    expect(spy.mock.calls).toHaveLength(1);
                    expect(spy.mock.calls[0]).toHaveLength(1);
                ...
                it('sets the state to cancelled', () => {
                    const { callInvite } = eventTest(mockNativeCallInviteEvents.cancelled, CallInvite.Event.Cancelled);
            ...
            describe(mockNativeCallInviteEvents.notificationTapped.type, () => {
                it('re-emits a valid event', () => {
                    const { spy } = eventTest(mockNativeCallInviteEvents.notificationTapped, CallInvite.Event.NotificationTapped);
                    expect(spy.mock.calls).toEqual([[]]);
                });
            });
            describe(mockNativeCallInviteEvents.rejected.type, () => {
                it('re-emits a valid event', () => {
                    const { spy } = eventTest(mockNativeCallInviteEvents.rejected, CallInvite.Event.Rejected);
                    expect(spy.mock.calls).toEqual([[]]);
                });
                it('sets the state to rejected', () => {
                    const { callInvite } = eventTest(mockNativeCallInviteEvents.rejected, CallInvite.Event.Rejected);
            ...
            describe(mockNativeCallInviteEvents.messageReceived.type, () => {
                it('constructs and emits a call message', () => {
                    const { spy } = eventTest(mockNativeCallInviteEvents.messageReceived, CallInvite.Event.MessageReceived);
                    expect(spy.mock.calls).toHaveLength(1);
        ...
        describe('when the event is invalid', () => {
            it('should throw an error containing the event type', () => {
                ...
                expect(() => MockNativeEventEmitter.emit(Constants.ScopeCallInvite, {
                    type: 'foo',
            ...
            it('should throw if the event is not an object', () => {
                // eslint-disable-next-line no-new
                new CallInvite(createNativeCallInviteInfo(), CallInvite.State.Pending);
                expect(() => MockNativeEventEmitter.emit(Constants.ScopeCallInvite, 'foobar')).toThrowError('Received a "string" native call invite event.');
            });
            it('should throw if the event is null', () => {
                // eslint-disable-next-line no-new
                new CallInvite(createNativeCallInviteInfo(), CallInvite.State.Pending);
                expect(() => MockNativeEventEmitter.emit(Constants.ScopeCallInvite, null)).toThrowError('Received a null native call invite event.');
            });
            it('should throw if the event does not have a callsid', () => {
                ...
                expect(() => MockNativeEventEmitter.emit(Constants.ScopeCallInvite, {
                    type: 'foo',
                })).toThrowError('Received a native call invite event without a call SID.');
            });
        });
    });
    describe.each([
        ...
        it('invokes the native module', async () => {
            await new CallInvite(createNativeCallInviteInfo(), CallInvite.State.Pending).accept(acceptOptions);
            expect(jest.mocked(MockNativeModule.callInvite_accept).mock.calls).toEqual([[createNativeCallInviteInfo().uuid, expectation]]);
        });
        ...
        it('returns a Promise<Call>', async () => {
            const callPromise = new CallInvite(createNativeCallInviteInfo(), CallInvite.State.Pending).accept(acceptOptions);
            await expect(callPromise).resolves.toBeInstanceOf(MockCall);
        });
        it('rejects when the native module rejects', async () => {
            const callPromise = new CallInvite(createNativeCallInviteInfo(), CallInvite.State.Pending);
        ...
        [
            ...
            const testMessage = `${shouldPass ? 'resolves' : 'rejects'} when the CallInvite state is "${callInviteState}"`;
            async function shouldResolve() {
                await expect(new CallInvite(createNativeCallInviteInfo(), callInviteState).accept(acceptOptions)).resolves.toBeInstanceOf(MockCall);
            }
            async function shouldReject() {
                const acceptPromise = new CallInvite(createNativeCallInviteInfo(), callInviteState).accept(acceptOptions);
                await expect(acceptPromise).rejects.toBeInstanceOf(MockInvalidStateError);
    ...
    describe('.reject()', () => {
        it('invokes the native module', async () => {
            await new CallInvite(createNativeCallInviteInfo(), CallInvite.State.Pending).reject();
            expect(jest.mocked(MockNativeModule.callInvite_reject).mock.calls).toEqual([[createNativeCallInviteInfo().uuid]]);
        });
        [
            [CallInvite.State.Accepted, false],
            [CallInvite.State.Rejected, false],
            ...
            const testMessage = `${shouldPass ? 'resolves' : 'rejects'} when the CallInvite state is "${callInviteState}"`;
            async function shouldResolve() {
                await expect(new CallInvite(createNativeCallInviteInfo(), callInviteState).reject()).resolves.toBeUndefined();
            }
            async function shouldReject() {
                const rejectPromise = new CallInvite(createNativeCallInviteInfo(), callInviteState).reject();
                await expect(rejectPromise).rejects.toBeInstanceOf(MockInvalidStateError);
                expect(MockInvalidStateError.mock.instances).toHaveLength(1);
                expect(MockInvalidStateError.mock.calls).toEqual([
                    [
                        `Call in state "${callInviteState}", ` +
    ...
    describe('.isValid()', () => {
        it('invokes the native module', async () => {
            await new CallInvite(createNativeCallInviteInfo(), CallInvite.State.Pending).isValid();
            expect(jest.mocked(MockNativeModule.callInvite_isValid).mock.calls).toEqual([[createNativeCallInviteInfo().uuid]]);
        });
        it('returns a Promise<boolean>', async () => {
            expect(typeof (await new CallInvite(createNativeCallInviteInfo(), CallInvite.State.Pending).isValid())).toBe('boolean');
        });
    });
    describe('.getCallSid()', () => {
        it('returns the callSid', () => {
            const callSid = new CallInvite(createNativeCallInviteInfo(), CallInvite.State.Pending).getCallSid();
    ...
    describe('.getCustomParameters()', () => {
        it('returns the custom parameters', () => {
            const customParameters = new CallInvite(createNativeCallInviteInfo(), CallInvite.State.Pending).getCustomParameters();
            expect(typeof customParameters).toBe('object');
            expect(customParameters).toEqual(createNativeCallInviteInfo().customParameters);
        });
    });
    describe('.getFrom()', () => {
        it('returns the from value', () => {
            const from = new CallInvite(createNativeCallInviteInfo(), CallInvite.State.Pending).getFrom();
    ...
    describe('.getState()', () => {
        it('returns the state', () => {
            const state = new CallInvite(createNativeCallInviteInfo(), CallInvite.State.Pending).getState();
    ...
    describe('.getTo()', () => {
        it('returns the to value', () => {
            const to = new CallInvite(createNativeCallInviteInfo(), CallInvite.State.Pending).getTo();
    ...
    describe('.sendMessage()', () => {
        ...
        describe('when invoking the native module', () => {
            it('stringifys content that is not a string', async () => {
                await new CallInvite(createNativeCallInviteInfo(), CallInvite.State.Pending).sendMessage({
                    content,
                    contentType,
                    messageType,
                });
        ...
        it('returns a Promise<OutgoingCallMessage>', async () => {
            ...
            const sendMessagePromise = new CallInvite(createNativeCallInviteInfo(), CallInvite.State.Pending).sendMessage(message);
            const mockResult = new OutgoingCallMessage({
                content,
                contentType,
                messageType,
    ...
    describe('private methods', () => {
         ...
         * Invalid event tests.
         */
        ['_handleNativeCallInviteEvent', '_handleMessageReceivedEvent'].forEach((privateMethodKey) => {
            describe(`.${privateMethodKey}`, () => {
                it('throws an error for an invalid event', () => {
                    const handler = new CallInvite(createNativeCallInviteInfo(), CallInvite.State.Pending)[privateMethodKey];
    ...
    describe('.updateCallerHandle()', () => {
        it('should resolve on ios platforms', async () => {
            const platformSpy = jest
                ...
                .mockReturnValue('ios');
            const nativeMethodSpy = jest.spyOn(NativeModule, 'callInvite_updateCallerHandle');
            await expect(new CallInvite(createNativeCallInviteInfo(), CallInvite.State.Pending).updateCallerHandle('foobar')).resolves.toBeUndefined();
        ...
        it('should reject on android platforms', async () => {
            ...
            const nativeMethodSpy = jest.spyOn(NativeModule, 'callInvite_updateCallerHandle');
            await expect(new CallInvite(createNativeCallInviteInfo(), CallInvite.State.Pending).updateCallerHandle('foobar')).rejects.toThrowError('Unsupported platform "android". This method is only supported on iOS.');
            expect(platformSpy.mock.calls).toEqual([[], []]);
            expect(nativeMethodSpy.mock.calls).toEqual([]);
        });
    });
