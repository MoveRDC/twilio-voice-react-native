...
const TYPED_ERRORS = ERRORS;
const getNamesFromExport = () => {
    return Object.values(TwilioErrors).reduce((reduction, errorOrNamespace) => {
        switch (typeof errorOrNamespace) {
            ...
            case 'object':
                const errorNames = Object.values(errorOrNamespace).map((errorConstructorOrConstant) => {
...
describe('generated errors', () => {
    it('contains all the expected error classes', () => {
        ...
        const generatedErrorNames = ErrorNamespaces.flatMap(([namespace, namespaceErrors]) => {
    ...
    for (const [code, ErrorClass] of errorsByCode.entries()) {
        describe(`${ErrorClass.name} - ${code}`, () => {
            ...
            it('defaults the message to the explanation', () => {
                let error = null;
                expect(() => (error = new ErrorClass(undefined))).not.toThrow();
...
describe('errorsByCode', () => {
    ...
    it('contains an entry for every exported error', () => {
        const namesFromMap = Array.from(errorsByCode.values()).map((errorConstructor) => errorConstructor.name);
        expect(namesFromMap.sort()).toStrictEqual(getNamesFromExport().namespaced.sort());
    });
});
