...
describe('IncomingCallMessage class', () => {
    describe('constructor', () => {
        it('uses the passed CallMessage info', () => {
            const incomingCallMessage = new IncomingCallMessage(createNativeCallMessageInfo());
            const callMessageInfo = createNativeCallMessageInfo();
            // eslint-disable-next-line dot-notation
            expect(incomingCallMessage['_content']).toStrictEqual(JSON.stringify(callMessageInfo.content));
            // eslint-disable-next-line dot-notation
            expect(incomingCallMessage['_contentType']).toStrictEqual(callMessageInfo.contentType);
            // eslint-disable-next-line dot-notation
            expect(incomingCallMessage['_messageType']).toStrictEqual(callMessageInfo.messageType);
            // eslint-disable-next-line dot-notation
            expect(incomingCallMessage['_voiceEventSid']).toStrictEqual(callMessageInfo.voiceEventSid);
        });
        const content = { key1: 'hello world' };
        const contentType = 'application/json';
        const messageType = 'user-defined-message';
        it('throws an error if "content" is undefined', () => {
            expect(() => new IncomingCallMessage({
                content: undefined,
                contentType,
                messageType,
        ...
        it('throws an error if "content" is null', () => {
            expect(() => new IncomingCallMessage({ content: null, contentType, messageType })).toThrowError('"content" must be defined and not "null".');
        });
        it('throws an error if "contentType" is not a valid string', () => {
            expect(() => new IncomingCallMessage({
                ...
                contentType: 10,
                messageType,
            })).toThrowError('If "contentType" is present, it must be of type "string".');
        });
    ...
    describe('.getContent()', () => {
        it('returns the content value', () => {
            const content = new IncomingCallMessage(createNativeCallMessageInfo()).getContent();
            expect(content).toEqual(JSON.stringify(createNativeCallMessageInfo().content));
        });
    });
    describe('.getContentType()', () => {
        it('returns the contentType value', () => {
            const contentType = new IncomingCallMessage(createNativeCallMessageInfo()).getContentType();
    ...
    describe('.getMessageType()', () => {
        it('returns the messageType value', () => {
            const messageType = new IncomingCallMessage(createNativeCallMessageInfo()).getMessageType();
    ...
    describe('.getSid()', () => {
        it('returns the voiceEventSid value', () => {
            const voiceEventSid = new IncomingCallMessage(createNativeCallMessageInfo()).getSid();
