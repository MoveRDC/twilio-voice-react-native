import { createNativeCallMessageInfo, createNativeCallMessageInfoSid, mockCallMessageNativeEvents, } from '../../__mocks__/CallMessage';
...
describe('OutgoingCallMessage class', () => {
    describe('constructor', () => {
        it('uses the passed CallMessage info', () => {
            const outgoingCallMessage = new OutgoingCallMessage(createNativeCallMessageInfo());
            const callMessageInfo = createNativeCallMessageInfo();
            // eslint-disable-next-line dot-notation
            expect(outgoingCallMessage['_content']).toStrictEqual(JSON.stringify(callMessageInfo.content));
            // eslint-disable-next-line dot-notation
            expect(outgoingCallMessage['_contentType']).toStrictEqual(callMessageInfo.contentType);
            // eslint-disable-next-line dot-notation
            expect(outgoingCallMessage['_messageType']).toStrictEqual(callMessageInfo.messageType);
            // eslint-disable-next-line dot-notation
            expect(outgoingCallMessage['_voiceEventSid']).toStrictEqual(callMessageInfo.voiceEventSid);
        });
    ...
    describe('.getContent()', () => {
        it('returns the outgoingCallMessageContent value', () => {
            const content = new OutgoingCallMessage(createNativeCallMessageInfo()).getContent();
            expect(content).toEqual(JSON.stringify(createNativeCallMessageInfo().content));
        });
    });
    describe('.getContentType()', () => {
        it('returns the outgoingCallMessageContentType value', () => {
            const contentType = new OutgoingCallMessage(createNativeCallMessageInfo()).getContentType();
    ...
    describe('.getMessageType()', () => {
        it('returns the outgoingCallMessageType value', () => {
            const messageType = new OutgoingCallMessage(createNativeCallMessageInfo()).getMessageType();
    ...
    describe('.getSid()', () => {
        it('returns the outgoingVoiceEventSid value', () => {
            const voiceEventSid = new OutgoingCallMessage(createNativeCallMessageInfo()).getSid();
    ...
    describe('on receiving a valid native event', () => {
         ...
         */
        const listenerCalledWithSent = (listenerMock) => {
        ...
        ].forEach(([{ name, nativeEvent }, OutgoingCallMessageEvent, assertion, voiceEventSid,]) => {
            describe(name, () => {
                it('re-emits the native event', () => {
                    const outgoingCallMessage = new OutgoingCallMessage(createNativeCallMessageInfoSid(voiceEventSid));
                ...
                it('invokes the correct event handler', () => {
                    const outgoingCallMessage = new OutgoingCallMessage(createNativeCallMessageInfoSid(voiceEventSid));
                    const spy = jest.spyOn(outgoingCallMessage['_nativeEventHandler'], // eslint-disable-line dot-notation
                    nativeEvent.type);
                    MockNativeEventEmitter.emit(Constants.ScopeCallMessage, nativeEvent);
                    expect(spy).toHaveBeenCalledTimes(1);
                });
                it('does not re-emit if voiceEventSid does NOT match', () => {
                    const outgoingCallMessage = new OutgoingCallMessage(createNativeCallMessageInfoSid('incorrect-sid'));
                    const listenerMock = jest.fn();
                    outgoingCallMessage.on(OutgoingCallMessageEvent, listenerMock);
                    MockNativeEventEmitter.emit(Constants.ScopeCallMessage, nativeEvent);
    ...
    describe('private methods', () => {
         ...
         * Invalid event tests.
         */
        ['_handleNativeEvent', '_handleFailureEvent', '_handleSentEvent'].forEach((privateMethodKey) => {
            describe(`.${privateMethodKey}`, () => {
                it('throws an error for an invalid event', () => {
                    const handler = new OutgoingCallMessage(createNativeCallMessageInfo())[privateMethodKey];
