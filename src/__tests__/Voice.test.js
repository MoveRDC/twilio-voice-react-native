...
const MockNativeEventEmitter = NativeEventEmitter;
const MockNativeModule = jest.mocked(NativeModule);
let MockAudioDevice;
let MockCall;
let MockCallInvite;
let MockTwilioError;
let mockConstructTwilioError;
jest.mock('../common');
...
describe('Voice class', () => {
    ...
    describe('on receiving a valid native event', () => {
        Object.values(mockVoiceNativeEvents).forEach(({ name, nativeEvent }) => {
            describe(name, () => {
                it('handles valid events', () => {
                    ...
                    const intermediateHandlerSpy = MockNativeEventEmitter.expectListenerAndReturnSpy(0, Constants.ScopeVoice, voice['_handleNativeEvent'] // eslint-disable-line dot-notation
                    );
                    const handlerSpy = jest.spyOn(voice['_nativeEventHandler'] /* eslint-disable-line dot-notation */, nativeEvent.type);
        ...
        describe(Constants.VoiceEventAudioDevicesUpdated, () => {
            it('constructs AudioDevice objects', () => {
                ...
                MockNativeEventEmitter.emit(Constants.ScopeVoice, mockVoiceNativeEvents.audioDevicesUpdated.nativeEvent);
            ...
            it('emits AudioDevice objects', () => {
                ...
                const [audioDevices, selectedDevice] = listenerMock.mock.calls[0];
            ...
            [undefined, null].forEach((nativeSelectedDevice) => {
                it(`emits undefined when the native selected audio device info is ${nativeSelectedDevice === null ? 'null' : 'undefined'}`, () => {
                    const voice = new Voice();
                    const listenerMock = jest.fn();
                    voice.on(Voice.Event.AudioDevicesUpdated, listenerMock);
                    const nativeEvent = {
        ...
        describe(Constants.VoiceEventTypeValueIncomingCallInvite, () => {
            it('constructs a pending CallInvite', () => {
                new Voice(); // eslint-disable-line no-new
                MockNativeEventEmitter.emit(Constants.ScopeVoice, mockVoiceNativeEvents.callInvite.nativeEvent);
        ...
        describe(Constants.VoiceEventError, () => {
            it('emits an error', async () => {
                ...
                MockNativeEventEmitter.emit(Constants.ScopeVoice, mockVoiceNativeEvents.error.nativeEvent);
            ...
            it('constructs an error', () => {
                ...
                expect(MockTwilioError.mock.calls).toHaveLength(MockTwilioError.mock.instances.length);
            });
        });
        describe(Constants.VoiceEventRegistered, () => {
            it('emits the event', () => {
                ...
                MockNativeEventEmitter.emit(Constants.ScopeVoice, mockVoiceNativeEvents.registered.nativeEvent);
        ...
        describe(Constants.VoiceEventUnregistered, () => {
            it('emits the event', () => {
                ...
                MockNativeEventEmitter.emit(Constants.ScopeVoice, mockVoiceNativeEvents.unregistered.nativeEvent);
                expect(listenerMock.mock.calls).toEqual([[]]);
            });
        });
    });
    describe('on receiving an invalid native event', () => {
        it('throws an error', () => {
            new Voice(); // eslint-disable-line no-new
            expect(() => MockNativeEventEmitter.emit(Constants.ScopeVoice, {
                type: 'mock-voice-eventtype',
            })).toThrowError('Unknown voice event type received from the native layer: ' +
    ...
    describe('public methods', () => {
        describe('.connect', () => {
            ...
            const performPlatformAgnosticTest = (testTitle, testFn) => {
            ...
            performPlatformAgnosticTest('throws when token is not a string', async () => {
                for (const invalidToken of [undefined, null, {}, 101, false]) {
                    await expect(new Voice().connect(invalidToken, options)).rejects.toThrowError('Argument "token" must be of type "string".');
                }
            });
            performPlatformAgnosticTest('throws when params is defined and not an object', async () => {
                for (const invalidParams of ['string', 101, false]) {
                    options.params = invalidParams;
                    await expect(new Voice().connect(token, options)).rejects.toThrowError('Optional argument "params" must be undefined or of type ' +
                        '"object".');
                }
            });
            performPlatformAgnosticTest('throws when one or more params is not a string', async () => {
                for (const invalidParamValue of [{}, 101, false, []]) {
                    options.params = {
                        foo: invalidParamValue,
                        bar: 'baz',
                    };
                    await expect(new Voice().connect(token, options)).rejects.toThrowError('Voice.ConnectOptions.params["foo"] must be of type string');
                }
            });
            performPlatformAgnosticTest('throws when contactHandle is defined and not a string', async () => {
                for (const invalidContactHandle of [null, {}, 101, false]) {
                    options.contactHandle = invalidContactHandle;
                    await expect(new Voice().connect(token, options)).rejects.toThrowError('Optional argument "contactHandle" must be undefined or of type' +
                        ' "string".');
                }
            });
            performPlatformAgnosticTest('succeeds when params is explicitly undefined', async () => {
                options.params = undefined;
                await expect(new Voice().connect(token, options)).resolves.toBeTruthy();
            });
            performPlatformAgnosticTest('succeeds when contactHandle is explicitly undefined', async () => {
                options.contactHandle = undefined;
                await expect(new Voice().connect(token, options)).resolves.toBeTruthy();
            });
            performPlatformAgnosticTest('succeeds when options are not passed', async () => {
                await expect(new Voice().connect(token)).resolves.toBeTruthy();
            });
            performPlatformAgnosticTest('returns a Promise<Call>', async () => {
                await expect(new Voice().connect(token, options)).resolves.toBeInstanceOf(MockCall);
            });
            describe('android platform', () => {
                ...
                it('invokes the proper native function', async () => {
                    ...
                    expect(jest.mocked(MockNativeModule.voice_connect_android).mock.calls).toEqual([[token, options.params]]);
                    expect(jest.mocked(MockNativeModule.voice_connect_ios).mock.calls).toEqual([]);
                });
                ...
                it('defaults params to "{}" when not passed', async () => {
                    ...
                    await new Voice().connect(token, options);
                    expect(jest.mocked(MockNativeModule.voice_connect_android).mock.calls).toEqual([[token, {}]]);
                });
            ...
            describe('ios platform', () => {
                ...
                it('invokes the proper native function', async () => {
                    await new Voice().connect(token, options);
                    expect(jest.mocked(MockNativeModule.voice_connect_ios).mock.calls).toEqual([[token, options.params, options.contactHandle]]);
                    expect(jest.mocked(MockNativeModule.voice_connect_android).mock.calls).toEqual([]);
                });
                it('defaults to "Default Contact" if contactHandle is an empty ' +
                    'string', async () => {
                    options.contactHandle = '';
                    await new Voice().connect(token, options);
                    expect(jest.mocked(MockNativeModule.voice_connect_ios).mock.calls).toEqual([[token, options.params, 'Default Contact']]);
                });
                ...
                it('defaults to "Default Contact" if contactHandle is ' +
                    'explicitly undefined', async () => {
                    options.contactHandle = undefined;
                    await new Voice().connect(token, options);
                    expect(jest.mocked(MockNativeModule.voice_connect_ios).mock.calls).toEqual([[token, options.params, 'Default Contact']]);
                });
                it('rejects when the native layer rejects', async () => {
                    ...
                    jest
                        ...
                        .mockRejectedValueOnce(someMockError);
                    await expect(() => new Voice().connect(token)).rejects.toThrow(someMockErrorMessage);
                });
                it('defaults params to "{}" when not passed', async () => {
                    delete options.params;
                    await new Voice().connect(token, options);
                    expect(jest.mocked(MockNativeModule.voice_connect_ios).mock.calls).toEqual([[token, {}, options.contactHandle]]);
                });
                it('defaults params to "{}" when params is explicitly undefined', async () => {
                    options.params = undefined;
                    await new Voice().connect(token, options);
            ...
            describe('unsupported platforms', () => {
                ...
                it('rejects', async () => {
                    await expect(() => new Voice().connect(token, options)).rejects.toThrowError(`Unsupported platform "${platform}". Expected "android" or "ios".`);
                    expect(jest.mocked(MockNativeModule.voice_connect_android).mock.calls).toEqual([]);
                    expect(jest.mocked(MockNativeModule.voice_connect_ios).mock.calls).toEqual([]);
                });
            });
        ...
        describe('.getVersion', () => {
            it('invokes the native module', () => {
                ...
                expect(jest.mocked(MockNativeModule.voice_getVersion).mock.calls).toEqual([[]]);
            });
        ...
        describe('.getDeviceToken', () => {
            it('invokes the native module', () => {
                ...
                expect(jest.mocked(MockNativeModule.voice_getDeviceToken).mock.calls).toEqual([[]]);
            });
            it('returns a Promise<string>', async () => {
                const deviceTokenPromise = new Voice().getDeviceToken();
                await expect(deviceTokenPromise).resolves.toBe('mock-nativemodule-devicetoken');
            });
        });
        describe('.getCalls', () => {
        ...
        describe('.getCallInvites', () => {
            it('invokes the native module', async () => {
                ...
                expect(jest.mocked(MockNativeModule.voice_getCallInvites).mock.calls).toEqual([[]]);
            });
        ...
        describe('.register', () => {
            it('invokes the native module', async () => {
                ...
                expect(jest.mocked(MockNativeModule.voice_register).mock.calls).toEqual([['mock-voice-token']]);
            });
        ...
        describe('.unregister', () => {
            it('invokes the native module', async () => {
                ...
                expect(jest.mocked(MockNativeModule.voice_unregister).mock.calls).toEqual([['mock-voice-token']]);
            });
            it('returns a Promise<void>', async () => {
        ...
        describe('.getAudioDevice', () => {
            it('invokes the native module', async () => {
                ...
                expect(jest.mocked(MockNativeModule.voice_getAudioDevices).mock.calls).toEqual([[]]);
            });
            it('returns a Promise resolving with audio devices info', async () => {
                const { audioDevices, selectedDevice } = await new Voice().getAudioDevices();
                expect(Array.isArray(audioDevices)).toBe(true);
                const allDevices = selectedDevice === null
                    ? audioDevices
        ...
        describe('.showAvRoutePickerView', () => {
            it('invokes the native module', async () => {
                ...
                expect(jest.mocked(MockNativeModule.voice_showNativeAvRoutePicker).mock.calls).toEqual([[]]);
            });
            it('returns a Promise<void>', async () => {
                const showAvRoutePickerViewPromise = new Voice().showAvRoutePickerView();
        ...
        describe('.initializePushRegistry', () => {
            it('should reject on android', async () => {
                jest.spyOn(Platform, 'OS', 'get').mockReturnValue('android');
                await expect(new Voice().initializePushRegistry()).rejects.toThrowError(UnsupportedPlatformError);
            });
            it('should resolve on ios', async () => {
                jest.spyOn(Platform, 'OS', 'get').mockReturnValue('ios');
                await expect(new Voice().initializePushRegistry()).resolves.toBeUndefined();
            });
        });
        describe('.setCallKitConfiguration', () => {
            ...
            it('should reject on android', async () => {
                jest.spyOn(Platform, 'OS', 'get').mockReturnValue('android');
                await expect(new Voice().setCallKitConfiguration(mockConfig)).rejects.toThrowError(UnsupportedPlatformError);
            });
            it('should resolve on ios', async () => {
                jest.spyOn(Platform, 'OS', 'get').mockReturnValue('ios');
                await expect(new Voice().setCallKitConfiguration(mockConfig)).resolves.toBeUndefined();
            });
        });
        describe('.setIncomingCallContactHandleTemplate', () => {
            it('invokes the native module with a string', async () => {
                ...
                expect(jest.mocked(MockNativeModule.voice_setIncomingCallContactHandleTemplate).mock.calls).toEqual([[template]]);
            });
            it('invokes the native module with no parameter', async () => {
                await new Voice().setIncomingCallContactHandleTemplate();
                expect(jest.mocked(MockNativeModule.voice_setIncomingCallContactHandleTemplate).mock.calls).toEqual([[]]);
            });
            it('returns a Promise<void>', async () => {
                const setIncomingCallContactHandleTemplatePromise = new Voice().setIncomingCallContactHandleTemplate('foobar');
                await expect(setIncomingCallContactHandleTemplatePromise).resolves.toBeUndefined();
            });
        });
        describe('.isFullScreenNotificationEnabled', () => {
            it('invokes the native module', async () => {
                ...
                expect(jest.mocked(MockNativeModule.system_isFullScreenNotificationEnabled)
                    .mock.calls).toEqual([[]]);
            });
        ...
        describe('.requestFullScreenNotificationPermission', () => {
            it('invokes the native module', async () => {
                ...
                expect(jest.mocked(NativeModule.system_requestFullScreenNotificationPermission).mock.calls).toEqual([[]]);
            });
        ...
        [
            ...
            describe(`.${privateMethodKey}`, () => {
                it('throws an error for an invalid event', () => {
                    const handler = new Voice()[privateMethodKey];
