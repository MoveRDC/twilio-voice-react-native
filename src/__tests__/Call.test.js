...
describe('Call class', () => {
    ...
    describe('on receiving a valid native event', () => {
        ...
        Object.values(mockCallNativeEvents).forEach(({ name, nativeEvent }) => {
            describe(name, () => {
                it('handles events for matching uuids', () => {
                    const call = new Call(createNativeCallInfo());
                    const intermediateSpy = MockNativeEventEmitter.expectListenerAndReturnSpy(0, Constants.ScopeCall, call['_handleNativeEvent'] // eslint-disable-line dot-notation
                    );
                    const handlerSpy = jest.spyOn(call['_nativeEventHandler'], // eslint-disable-line dot-notation
                    nativeEvent.type);
                ...
                it('updates the call info', () => {
                    ...
                    const call = new Call(createNativeCallInfo());
                    const updateSpy = jest.spyOn(call, '_update');
         ...
         */
        const listenerCalledWithoutArgs = (listenerMock) => {
            expect(listenerMock).toHaveBeenCalledTimes(1);
            expect(listenerMock).toHaveBeenNthCalledWith(1);
        };
        const listenerCalledWithGenericError = (listenerMock) => {
            expect(listenerMock).toHaveBeenCalledTimes(1);
        ...
        const listenerCalledWithQualityWarnings = (listenerMock) => {
            expect(listenerMock).toHaveBeenCalledTimes(1);
            const args = listenerMock.mock.calls[0];
        ...
        const listenerCalledWithMessageReceived = (listenerMock) => {
            expect(listenerMock).toHaveBeenCalledTimes(1);
            const args = listenerMock.mock.calls[0];
        ...
        [
            ...
            describe(name, () => {
                ...
                it('invokes the correct event handler', () => {
                    const call = new Call(createNativeCallInfo());
                    const spy = jest.spyOn(call['_nativeEventHandler'], // eslint-disable-line dot-notation
                    nativeEvent.type);
                    MockNativeEventEmitter.emit(Constants.ScopeCall, nativeEvent);
        ...
        describe(Constants.CallEventQualityWarningsChanged, () => {
            it('does not update the call state', () => {
                ...
                MockNativeEventEmitter.emit(Constants.ScopeCall, mockCallNativeEvents.connected.nativeEvent);
                expect(updateSpy).toHaveBeenCalledTimes(1);
                expect(call.getState()).toBe(Call.State.Connected);
                MockNativeEventEmitter.emit(Constants.ScopeCall, mockCallNativeEvents.qualityWarningsChanged.nativeEvent);
    ...
    describe('uses the error constructor', () => {
        [
            ...
            it(ev, () => {
                ...
                expect(mockConstructTwilioError.mock.calls).toHaveLength(mockConstructTwilioError.mock.instances.length);
            });
        });
    });
    /**
    ...
    describe('on receiving an invalid native event', () => {
        it('throws an error', () => {
            new Call(createNativeCallInfo()); // eslint-disable-line no-new
            expect(() => MockNativeEventEmitter.emit(Constants.ScopeCall, {
                type: 'mock-call-eventtype',
            })).toThrowError('Unknown call event type received from the native layer: ' +
                '"mock-call-eventtype".');
        });
    });
    describe('public methods', () => {
        describe('.disconnect', () => {
            it('invokes the native module', async () => {
                ...
                expect(jest.mocked(MockNativeModule.call_disconnect).mock.calls).toEqual([['mock-nativecallinfo-uuid']]);
            });
        ...
        describe('.getCustomParameters', () => {
            it('returns the customParameters value', () => {
                const customParameters = new Call(createNativeCallInfo()).getCustomParameters();
                expect(customParameters).toEqual({
                    'mock-nativecallinfo-custom-parameter-key1': 'mock-nativecallinfo-custom-parameter-value1',
                    'mock-nativecallinfo-custom-parameter-key2': 'mock-nativecallinfo-custom-parameter-value2',
                });
            });
        });
        describe('.getFrom', () => {
        ...
        describe('.sendDigits', () => {
            it('invokes the native module', async () => {
                ...
                expect(jest.mocked(MockNativeModule.call_sendDigits).mock.calls).toEqual([['mock-nativecallinfo-uuid', digits]]);
            });
            it('returns a Promise<void>', async () => {
                const sendDigitsPromise = new Call(createNativeCallInfo()).sendDigits('12345');
        ...
        describe('.postFeedback', () => {
            it('invokes the native module', async () => {
                ...
                expect(jest.mocked(MockNativeModule.call_postFeedback).mock.calls).toEqual([
                    [
                        'mock-nativecallinfo-uuid',
            ...
            it('rejects when passing an invalid score', async () => {
                ...
                const call = new Call(createNativeCallInfo());
                await call
                    .postFeedback('foobar', Call.Issue.Echo)
                    .catch((error) => {
                    expect(error).toBeInstanceOf(InvalidArgumentError);
                    expect(error.message).toStrictEqual('"score" parameter invalid. ' +
            ...
            it('rejects when passing an invalid issue', async () => {
                ...
                const call = new Call(createNativeCallInfo());
                await call
                    .postFeedback(Call.Score.Three, 'foobar')
                    .catch((error) => {
                    expect(error).toBeInstanceOf(InvalidArgumentError);
                    expect(error.message).toStrictEqual('"issue" parameter invalid. ' +
            ...
            it('returns a Promise<void>', async () => {
                const issue = Call.Issue.AudioLatency;
                const score = Call.Score.Three;
                const postFeedbackPromise = new Call(createNativeCallInfo()).postFeedback(score, issue);
        ...
        describe('.sendMesssage', () => {
            ...
            it('returns a Promise<OutgoingCallMessage>', async () => {
                ...
                const sendMessagePromise = new Call(createNativeCallInfo()).sendMessage(message);
        ...
        [
            ...
            describe(`.${privateMethodKey}`, () => {
                it('throws an error for an invalid event', () => {
                    const handler = new Call(createNativeCallInfo())[privateMethodKey];
